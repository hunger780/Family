        ...
        <div className="flex-1 overflow-y-auto">
          {chats.map(chat => (
            <button 
              ...
              className={`w-full p-4 flex items-center gap-3 hover:bg-slate-800 transition-colors ${activeChat === chat.id ? 'bg-slate-800 border-l-2 border-pink-500' : ''}`}
            >
      ...
      <div className="flex-1 flex flex-col bg-slate-900/50">
        <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-white">Family Group ❤️</h3>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.map(msg => (
            <div key={msg.id} className={`flex ${msg.isMe ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[70%] ${msg.isMe ? 'bg-pink-600 text-white rounded-l-2xl rounded-tr-2xl' : 'bg-slate-800 text-slate-200 rounded-r-2xl rounded-tl-2xl'} p-4 shadow-md`}>
